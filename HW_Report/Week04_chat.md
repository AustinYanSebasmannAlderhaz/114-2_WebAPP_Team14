...
> 我再做一版更強韌的修正：把時間線卡片圖片從「固定 450px」改成「容器自適應」，避免任何手機寬度下再撐爆，並加上文字區保底設定，確保內容一定可見。現在直接更新 style.css。
>
...
修正重點：
>
>移除時間線圖片固定 450px，改成自適應寬度（width: 100%）
era-block 手機版改為上下排列（包含 .reverse）
手機版圖片限制為 height: 200px; object-fit: cover，不再撐爆版面
補上文字區 min-width: 0 與 word-break: break-word，避免文字被擠壓看不到
這樣手機上會是「圖片在上、文字在下」，文字可完整顯示。
...
>
>我找到問題點了：characters.html 的卡片仍是一般 <div class="card"><img ...> 結構，沒有套到你在 style.css 裡為角色卡設計的 .character-card / .card-bg-img / .card-content 樣式，所以圖片會顯示異常。
我會把這頁卡片結構改成和首頁角色卡一致來修復。
>
...
修正內容：
>
>卡片改為 card character-card
圖片改為 card-bg-img（並套 kai-img / lloyd-img 定位）
文字改為包在 card-content（對應半透明底與覆蓋樣式）
導覽列的「角色」加上 active 狀態
這樣圖片就會回到正確裁切與覆蓋排版。
